...
def parse_proxy(proxy_str):
    ...
    elif len(parts) == 2 and parts[0].replace('.', '').isdigit() and parts[1].isdigit():
        # host:port (no auth)
        return f"http://{proxy_str}"
    else:
        # Fallback: treat entire string as host (maybe with port using : )
        if ':' in proxy_str:
            # Assume host:port
            return f"http://{proxy_str}"
        else:
            # Just host, assume port 8080?
...
# ---------- ShopifyChecker class (modified for proxy) ----------
class ShopifyChecker:
    ...
    def check_card(self, site, cc_line):
        # Re-init with proxy (if any) but keep base_url
        self.__init__(base_url=site, proxy=self.session.proxies.get('http') if self.session.proxies else None)
...
@app.get("/")
async def root():
    return {"message": "Shopify Checker API is running. Use /shopify?site=...&cc=...&proxy=..."}

@app.get("/shopify", response_model=CheckResponse)
async def shopify_check(
    site: str = Query(..., description="Shopify store URL (e.g., https://store.myshopify.com)"),
    cc: str = Query(..., description="Card details: number|month|year|cvv"),
    proxy: str = Query(None, description="Optional proxy (supports format: host:port:user:pass or http://user:pass@host:port)")
):
    # Validate inputs
    if not site.startswith(('http://', 'https://')):
        site = 'https://' + site
    if '|' not in cc:
        raise HTTPException(status_code=400, detail="Invalid card format. Must be number|month|year|cvv")
    
    # Parse proxy if provided
    parsed_proxy = parse_proxy(proxy) if proxy else None
    logger.info(f"Checking card on {site} with proxy {parsed_proxy}")
    
    checker = ShopifyChecker(base_url=site, proxy=parsed_proxy)
    category, cc_out, detail = checker.check_card(site, cc)
    
    # Save result to files (optional)
...
# ---------- CLI mode (optional) ----------
# (You can keep the original CLI functions here if you want to run locally)
