...
async def shopify_check(
    ...
    proxy: str = Query(None, description="Optional proxy URL (http://user:pass@host:port)")
):
    # Validate inputs
    ...
    logger.info(f"Checking card on {site} with proxy {proxy}")
...
# ---------- CLI mode (original) ----------
# ... (keep the original CLI functions if you like, but not required for API)
# For Railway, we run uvicorn in the Procfile.
